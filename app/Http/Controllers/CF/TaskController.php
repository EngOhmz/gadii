...
{  
    ...
    {  
        ...
        return view('cf.task.index', compact('project', 'categories','billing_type','task', 'milestones', 'leads', 'users', 'reference'));
    }
              ...
              $comment_details=Comment::where('task_id',$id)->where('disabled','0')->where('comments_reply_id', '0')->orderBy('comment_datetime', 'DESC')->get();                
               ...
               $actcount=TaskActivity::where('task_id',$id)->count();   
                ...
                return view('project.task.project_details',compact
                  ('data','id','type','comment_details','attach','notes','activity','task','categories','milestones','users',
                    'ccount', 'attcount','ncount', 'actcount', 'tcount'));
    }
...
if(!empty($meet)){
         ...
         return redirect(route('cf_task.show',$request->task_id))->with(['success'=>"Details Created Successfully",'type'=>'attachment']);
                   break;     
...
if(!empty($note)){
         ...
         return redirect(route('cf_task.show',$request->task_id))->with(['success'=>"Details Created Successfully",'type'=>'notes']);
                   break;    
        ...
        switch ($type) {
           case 'edit-attachment':
              ...
              $name =Items::where('added_by',auth()->user()->added_by)->get() ; 
               ...
               return view('project.task.project_details',compact('data','id','type','type_id','edit_data','name','chart_of_accounts', 'activity', 'notes', 'comment_details', 'attach', 'task', 'leads', 'project', 'categories','milestones', 'billing_type',
                            'ccount', 'attcount','ncount', 'actcount', 'tcount'));
          ...
          case 'edit-notes':        
             ...
             return view('project.task.project_details',compact('data','id','type','type_id','edit_data', 'activity', 'notes', 'comment_details', 'attach', 'task', 'leads', 'project', 'categories','milestones','users','name','chart_of_accounts', 'billing_type',
                            'ccount', 'attcount','ncount', 'actcount', 'tcount'));
...
if(!empty($meet)){
         ...
         return redirect(route('cf_task.show',$request->task_id))->with(['success'=>"Details Updated Successfully",'type'=>'notes']);
    ...
    public function discountModal(Request $request)
    {
                 ...
                 $type = $request->type;
                  ...
                  if($type == 'assign'){
                    ...
                    $data=Task::find($id);
                    return view('cf.task.view_user',compact('id','user','data'));
                }
