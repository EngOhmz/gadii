...
namespace App\Http\Controllers\Api_controllers\Pms;
...
class Auth_ApiController extends Controller
{
     ...
     public function login(Request $request)
    {
        ...
        $userRole =  User_Roles::where('user_id',$user->id)->value('role_id');
            ...
            if($userRole == 73 || $userRole == 56 || $userRole == 1){
    ...
    {
        $roles = Role::all()->whereIn('slug', ['Restaurant']);
    ...
    public function register(Request $request)
    {
        ...
        try {
            ...
            if($user){
                ...
                $roles_added2 = Role::where('id', 73)->first();
...
if(!empty($client)){
    ...
    $account_typeOld = DB::table('gl_account_typeOld')->get();
       ...
       $group=GroupAccount::where('name', $row->account_group)->where('added_by',$user->id)->first();
            ...
            $number = $user->phone;
        //   $message = "Welcome to EMASUITE PMS, $user->name . Your registration is successful. If you have any questions or need assistance, call us on +255 655 973 248. \n Powered by UjuziNet.";
          $message = "Dear  $user->name, Congratulations on joining EMASUITE PMS! Manage finances, inventories, staffs, operations, customers and more. Pay now or within 7 days. Assistance: +255 655 973 248. \n Powered by UjuziNet.";
          $option11 = 1;
          $type = "sms";
          $useRandomDevice = 1;
          $prioritize = 1;
