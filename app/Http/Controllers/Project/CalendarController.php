...
{  
    ...
    {  
        ...
        // $users = User::all()->where('added_by', auth()->user()->added_by);
         ...
         $projects = Project::all()->where('disabled','0')->where('added_by',auth()->user()->added_by);
         $task = Task::all()->where('added_by',auth()->user()->added_by)->where('disabled','0');
         $mile = Milestone::all()->where('disabled','0')->where('added_by',auth()->user()->added_by);
         $inv=Invoice::all()->where('invoice_status',1)->where('added_by',auth()->user()->added_by);
         $pur=Purchase::all()->where('disabled','0')->where('added_by',auth()->user()->added_by);
    
        return view('project.calendar.index', compact('projects','task','mile','inv','pur'));
