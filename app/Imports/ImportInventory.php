...
class ImportInventory implements ToCollection,WithHeadingRow,WithCalculatedFormulas
...
$old=Inventory::where(DB::raw('lower(name)'), strtolower($row['name']))->where('added_by',auth()->user()->added_by)->where('disabled','0')->first();  
...
$c=$a->id;
     ...
     Inventory::find($old->id)->update([
        'quantity' => $old->quantity + $row['quantity'],
        ]);
        
        $b= Inventory::find($old->id);

$c=$old->id;

}
      
   $item= Inventory::find($c) ;
 ...
 if($nameArr > 0){ 
     ...
     $drjournal= JournalEntry::create([
       ...
       'date' =>  $today, 
        ...
        'income_id' => $item->id,
        'name' => 'Inventory',
        'transaction_type' => 'import_inventory',
        'notes' => 'Inventory Balance for ' .$row['name'],
        'added_by' => auth()->user()->added_by,
        ]);
         ...
         'date' =>  $today, 
          ...
          'added_by' => auth()->user()->added_by,
                     ...
                     MasterHistory::create($mlists);
                        ...
                        for($x = 1; $x <= $nameArr; $x++){    
                        $name=Inventory::where('id', $item->id)->first();
