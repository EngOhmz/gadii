...
class ImportManufacturingItems  implements ToCollection,WithHeadingRow,WithCalculatedFormulas
...
//, WithValidation
   ...
   if($row['type'] == 'Finished Goods' || $row['type'] == 'Manufacture' ||$row['type'] == 'Manufacture(Finished Goods)'){
    $type=2;
   }
...
elseif($row['type'] == 'Semi Finished Goods'){
    $type=5;
...
$old=Items::where(DB::raw('lower(name)'), strtolower($row['name']))->where('added_by',auth()->user()->added_by)->where('disabled','0')->first();  
