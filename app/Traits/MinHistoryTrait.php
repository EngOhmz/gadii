...
trait MinHistoryTrait
{
    public function getItems($start_date = null, $end_date = null, $location = null)
    {
        $user = Auth::user();
        ...
        return $query->get()->map(function ($item) {
            ...
            // Determine the stock status based on quantity and minimum balance
            $quantity = $item->quantity;
            $minimum_balance = $itemDetails->minimum_balance;
            $status = '';

            if ($quantity >= 1 || $minimum_balance >= 1) {
                $status = 'In Stock';
            } elseif ($quantity < 1 && $quantity > 0 || $minimum_balance < 1 && $minimum_balance > 0) {
