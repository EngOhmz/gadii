...
<div class="card-body">
    <ul class="nav nav-tabs" id="myTab2" role="tablist">
        ...
        <li class="nav-item">
            <a class="nav-link @if ($type == 'edit-tasks') active @endif" id="profile-tab2" data-toggle="tab"
                href="#task-profile2" role="tab" aria-controls="profile" aria-selected="false">New Sub Tasks</a>
        </li>
    </ul>
    <br>
    <div class="tab-content tab-bordered" id="myTab3Content">
        <div class="tab-pane fade @if (
            $type == 'details' ||
                ...
                $type == 'tasks' ||
                $type == 'notes' ||
                $type == 'activities') active show @endif " id="task-home2" role="tabpanel"
                                    ...
                                    <td>
                                        <div class="form-inline">
                                            ...
                                            <a class="list-icons-item text-primary" title="Edit"
                                                href="{{ route('edit.task_details', ['type' => 'edit-tasks', 'type_id' => $row->id]) }}"><i
                                                    class="icon-pencil7"></i></a>&nbsp
                                            <a class="list-icons-item text-danger" title="Edit"
                                                href="{{ route('delete.task_details', ['type' => 'delete-tasks', 'type_id' => $row->id]) }}"
                    ...
                    <div class="row">
                        <div class="col-sm-12 ">
                            @if ($type == 'edit-tasks')
                                ...
                                {!! Form::open(['route' => 'update.task_details', 'enctype' => 'multipart/form-data']) !!}
                                <input type="hidden" name="id" value="{{ $type_id }}">
                            @else
                                {!! Form::open(['route' => 'save.task_details', 'enctype' => 'multipart/form-data']) !!}
                                @method('POST')
                            @endif
                                    ...
                                    <select class="m-b assign" id="assigned_to" name="assigned_to" required>
                                        ...
                                        @if (!empty($users))
                                            @foreach ($users as $row)
                                                <option value="{{ $row->id }}"
                                                    @if (isset($edit_data)) @if ($edit_data->assigned_to == $row->id) selected @endif
                                                    @endif >{{ $row->name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
