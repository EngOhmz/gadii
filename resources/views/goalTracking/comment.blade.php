...
<div class="card-body">
  ...
  <div class="row">
    <div class="col-sm-12 ">
      {!! Form::open(array('route' => 'save.goal_details',"enctype"=>"multipart/form-data")) !!}
      @method('POST')
    ...
    <div class="media-chat-scrollable mb-3">
      <div class="media-chat vstack gap-3">
        ...
        <div class="hstack align-items-start gap-3">
          ...
          <br>
          <button style="border:none;background: none;" class="text-primary reply" data-toggle="tooltip" data-placement="top"
          title="Click to reply" id="reply__<?php echo $v_comment->id ?>" data-sub_category_id="<?php echo $v_comment->id ?>"><i class="icon-reply"></i> Reply</button> &nbsp
          @if ($v_comment->user_id == auth()->user()->id)

          <a class="text-danger delete" title="Edit" href="{{route('delete.goal_details',['type'=>'delete-comments','type_id'=>$v_comment->id])}}"
          onclick="return confirm('Are you sure, you want to delete?')"><i class="icon-trash"></i> Delete</a>
          @endif



          <?php
          $comment_reply = App\Models\Goal_Tracking\Comment::where('goal_id', $id)->where('comments_reply_id', $v_comment->id)->where('disabled', '0')->orderBy('comment_datetime', 'ASC')->get();
          ?>
          ...
          <div class="reply__" id="reply_<?php echo $v_comment->id ?>" style="display: none;">
            <form id="projects-reply-comment-form-<?php echo $v_comment->id ?>" action="{{route('save.cf_details')}}" method="post">
              ...
              <button type="submit" id="reply-<?php echo $v_comment->id ?>" class="btn btn-xs btn-primary">Save</button>
