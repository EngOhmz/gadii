    ...
    <div class="section-body">
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-12 col-lg-12">
                 ...
                 @php  
                 $rn=App\Models\ReturnInvoice::where('invoice_id',$invoices->id)->where('added_by',auth()->user()->added_by)->first(); 
                 $today = date('Y-m-d');
               ...
               <div class="col-lg-10">
                @if ($today < $next)
                  ...
                  @if(empty($rn))
                 <a class="btn btn-xs btn-primary"  onclick="return confirm('Are you sure?')"   href="{{ route('inventory_invoice.edit', $invoices->id)}}"  title="" > Edit </a>                      
                @endif
                       @endcan
               ...
               @if($invoices->status != 0 && $invoices->status != 4 && $invoices->status != 3 && $invoices->good_receive == 1)                      
                <a class="btn btn-xs btn-info " data-placement="top"  href="{{ route('inventory_invoice.pay',$invoices->id)}}"  title="Add Payment"> Pay invoice  </a>    
           @endif  
 
             
             <a class="btn btn-xs btn-success"  href="{{ route('inventory_invoice_pdfview',['download'=>'pdf','id'=>$invoices->id]) }}"  title="" > Download PDF </a>
               <a class="btn btn-xs btn-success"  href="{{ route('inventory_invoice_receipt',['download'=>'pdf','id'=>$invoices->id]) }}"  title="" > Download Receipt </a>
                        ...
                        <th style="color:white;">Total</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td class="">{{$i++}}</td>
                                            <?php
                                         $item_name = App\Models\InventoryList::find($row->item_name);
                                        ?>
                                <td class=""><strong class="block">@if(!empty($item_name->serial_no)) {{$item_name->brand->name}} - {{$item_name->serial_no}}  @else {{$row->item_name}}  @endif   </strong>
                                <br>{{$row->description}}
                                </td>
...
<td colspan="5"></td>
<td>Discount</td>
<td>{{number_format($invoices->discount ,2)}}  {{$invoices->exchange_code}}</td>
</tr>
<tr>
    <td colspan="5"></td>
    <td>Total Amount</td>
    <td>{{number_format(($gland_total +  $invoices->shipping_cost)  - $invoices->discount ,2)}}  {{$invoices->exchange_code}}</td>
     ...
     @if(!empty($invoices->due_amount < ($invoices->invoice_amount + $invoices->invoice_tax +  $invoices->shipping_cost)  - $invoices->discount))
         <tr>
    <td colspan="5"></td>
                        <td>Paid Amount</p>
                        <td>{{number_format(( ($invoices->invoice_amount + $invoices->invoice_tax +  $invoices->shipping_cost)  - $invoices->discount) - $invoices->due_amount,2)}}  {{$invoices->exchange_code}}</p>
                    </tr>
...
<td colspan="5"></td>
<td>Total Amount</td>
<td>{{number_format($invoices->exchange_rate * ($gland_total-$invoices->discount) ,2)}}   {{$def->currency}}</td>
</tr>
...
?>
                                            ...
                                            <td><a  href="{{ route('inventory_invoice_payment_pdfview',['download'=>'pdf','id'=>$row->id]) }}"  title="" > Download Receipt </a> </td>
                                        </tr>
                                        @endforeach
