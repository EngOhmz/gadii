...
<script>
 ...
 $(function() {
    let urlcontract = "{{ route('inventory.good_issue_report') }}";
        ...
        buttons: [
          {extend: 'copyHtml5',title: 'GOOD ISSUE REPORT FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}} ', footer: true},
           ...
           {extend: 'csvHtml5',title: 'GOOD ISSUE REPORT FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}' , footer: true},
            {extend: 'pdfHtml5',title: 'GOOD ISSUE REPORT FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}', footer: true},
...
$('.datatable-button-html5-basic').on('xhr.dt', function(e, settings, json, xhr ) {
  json.data = json.data.filter(function(row) {
    ...
    var a=row.issue.replace(/<\/?a[^>]*>/g, '');
     var open = parseFloat(a);
     
     var b=row.return.replace(/<\/?a[^>]*>/g, '');
