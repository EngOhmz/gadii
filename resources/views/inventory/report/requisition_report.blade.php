...
<script>
 ...
 $(function() {
    let urlcontract = "{{ route('inventory.requisition_report') }}";
      ...
      $('.datatable-button-html5-basic').DataTable({
        ...
        "footerCallback": function ( tfoot, data, start, end, display ) {
            ...
            // Update footer by showing the total with the reference of the column index 
             $( api.column( 3 ).footer() ).html(numberWithCommas(openTotal.toFixed(2)));
            $( api.column( 4 ).footer() ).html(numberWithCommas(inTotal.toFixed(2)));
        ...
        buttons: [
          {extend: 'copyHtml5',title: 'REQUISITION REPORT  FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}} ', footer: true},
           {extend: 'excelHtml5',title: 'REQUISITION REPORT  FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}' , footer: true},
           {extend: 'csvHtml5',title: 'REQUISITION REPORT  FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}' , footer: true},
            {extend: 'pdfHtml5',title: 'REQUISITION REPORT  FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}', footer: true},
            {extend: 'print',title: 'REQUISITION REPORT  FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}' , footer: true}
     ...
     <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.item', function() {
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ url('reports/inventory/viewModal') }}',
                    data: {
                    'id': id,
                    'type': type,
