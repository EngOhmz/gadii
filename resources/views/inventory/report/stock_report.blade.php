...
<script>
 ...
 $(function() {
    let urlcontract = "{{ route('inventory.stock_report') }}";
        ...
        buttons: [
          {extend: 'copyHtml5',title: 'STOCK VALUE  FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}} ', footer: true},
           ...
           {extend: 'csvHtml5',title: 'STOCK VALUE  FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}' , footer: true},
            {extend: 'pdfHtml5',title: 'STOCK VALUE  FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}', footer: true},
...
$('.datatable-button-html5-basic').on('xhr.dt', function(e, settings, json, xhr ) {
  json.data = json.data.filter(function(row) {
    ...
    var a=row.open.replace(/<\/?a[^>]*>/g, '');
     ...
     var b=row.purchases.replace(/<\/?a[^>]*>/g, '');
