...
<section class="section">
  ...
  @endif
                                        ...
                                        @endif
                                                           ...
                                                           @if($row->good_receive == 1)
                                                            <li class="nav-item"><a class="nav-link" title="Delivery Note"
                                                                    href="{{ route('pharmacy_pos_note_pdfview',['download'=>'pdf','id'=>$row->id]) }}">Download Delivery Note</a></li>
                                        @endif
                                      ...
                                      @else
                                                    ...
                                                    <a class="list-icons-item text-primary"
                                                        title="Edit" onclick="return confirm('Are you sure?')"
                                                        href="{{ route('pharmacy_serial_invoice.edit', $row->id)}}"><i
                                                            class="icon-pencil7"></i></a>&nbsp
                                                          

                                                    {!! Form::open(['route' => ['pharmacy_serial_invoice.destroy',$row->id],
                                                    'method' => 'delete']) !!}
                                 {{ Form::button('<i class="icon-trash"></i>', ['type' => 'submit','style' => 'border:none;background: none;', 'class' => 'list-icons-item text-danger', 'title' => 'Delete', 'onclick' => "return confirm('Are you sure?')"]) }}
                                                    {{ Form::close() }}
                                 &nbsp
  @endif
                                                <div class="dropdown">
													...
													<div class="dropdown-menu">
                                                            @if($row->status == 0 && $row->status != 4 && $row->status != 3 && $row->good_receive == 0)
                                                            <li> <a class="nav-link" id="profile-tab2"
                                                                    href="{{ route('pharmacy_serial_invoice.receive',$row->id)}}"
                                                                    role="tab"
                                                                    aria-selected="false" onclick="return confirm('Are you sure?')">Approve Invoice</a>
                                                            </li>
                                                            @endif
                                                             @if($row->status != 0 && $row->status != 4 && $row->status != 3 && $row->good_receive == 1)
                                                            <li> <a class="nav-link" id="profile-tab2"
                                                                    href="{{ route('pharmacy_serial_pos_invoice.pay',$row->id)}}"
                                                                    role="tab"
                                                                    aria-selected="false">Make Payments</a>
                                                            </li>
                                                            ...
                                                            <li class="nav-item"><a class="nav-link" title="Cancel"
                                                                    onclick="return confirm('Are you sure?')"
                                                                    href="{{ route('pharmacy_serial_invoice.cancel', $row->id)}}">Cancel
                                                                  Invoice</a></li>
                                        @endif
                                                     <li class="nav-item"><a class="nav-link" title="PDF"
                                                                   href="{{ route('pharmacy_serial_pos_invoice_pdfview',['download'=>'pdf','id'=>$row->id]) }}">Download PDF</a></li>
													

                                                                      @if($row->good_receive == 1)
                                                            <li class="nav-item"><a class="nav-link" title="Delivery Note"
                                                                    href="{{ route('pharmacy_serial_pos_note_pdfview',['download'=>'pdf','id'=>$row->id]) }}">Download Delivery Note</a></li>
                                        @endif
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-sm-12 ">
                                                @if(isset($id))
                                                {{ Form::model($id, array('route' => array('pharmacy_invoice.update', $id), 'method' => 'PUT')) }}
                                              
                                                @else
                                                {{ Form::open(['route' => 'pharmacy_invoice.store']) }}
                                                @method('POST')
                                                @endif
