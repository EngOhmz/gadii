...
<script>
 ...
 $(function() {
    let urlcontract = "{{ route('pos.store_value') }}";
   ...
   */
	    ...
	    var outTotal = api
            ...
            $( api.column( 1 ).footer() ).html(inTotal.toFixed(2));
            $( api.column( 2 ).footer() ).html(outTotal.toFixed(2));
            $( api.column( 3 ).footer() ).html(balanceTotal.toFixed(2));
        },  
        ...
        buttons: [
          {extend: 'copyHtml5',title: 'STORE VALUE FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}} ', footer: true},
           ...
           {extend: 'csvHtml5',title: 'STORE VALUE FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}' , footer: true},
            {extend: 'pdfHtml5',title: 'STORE VALUE FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}', footer: true},
...
$('.datatable-button-html5-basic').on('xhr.dt', function(e, settings, json, xhr ) {
  json.data = json.data.filter(function(row) {
     ...
     var a=row.in.replace(/<\/?a[^>]*>/g, '');
     var open = parseFloat(a);
     
     var b=row.out.replace(/<\/?a[^>]*>/g, '');
