...
<script>
 ...
 $(function() {
    let urlcontract = "{{ route('pos.report_by_date') }}";
   ...
   */
	    ...
	    var purTotal = api
	     ...
	     var closeTotal = api
            ...
            $( api.column( 1 ).footer() ).html(openTotal.toFixed(2));
            $( api.column( 2 ).footer() ).html(purTotal.toFixed(2));
            $( api.column( 3 ).footer() ).html(salesTotal.toFixed(2));
            $( api.column( 4 ).footer() ).html(closeTotal.toFixed(2));
        },  
...
$('.datatable-button-html5-basic').on('xhr.dt', function(e, settings, json, xhr ) {
  json.data = json.data.filter(function(row) {
     ...
     var b=row.purchase.replace(/<\/?a[^>]*>/g, '');
     var purchase = parseFloat(b);
     
     var c=row.sales.replace(/<\/?a[^>]*>/g, '');
