                ...
                <div class="col-12 col-md-12 col-lg-12">
                    ...
                    <div class="col-lg-10">
                        ...
                        @if($invoices->status != 0 && $invoices->status != 4 && $invoices->status != 3 && $invoices->invoice_status == 1)
                            <a class="btn btn-xs btn-info " data-placement="top"  href="{{ route('pos_invoice.pay',$invoices->id)}}"  title="Add Payment"> Pay invoice  </a>
                        @endif

                        @if($invoices->invoice_status == 1 &&  $invoices->invoice_status == 0)
                        ...
                        <a class="btn btn-xs btn-info"  target="_blank" href="{{ route('pos_receipt_print',['download'=>'pdf','id'=>$invoices->id]) }}"  title="" > Print Receipt</a>

                        <a class="btn btn-xs btn-success"  href="{{ route('pos_invoice_pdfview',['download'=>'pdf','id'=>$invoices->id]) }}"  title="" > Download PDF </a>
                        <a class="btn btn-xs btn-success"  href="{{ route('pos_invoice_receipt',['download'=>'pdf','id'=>$invoices->id]) }}"  title="" > Download Receipt </a>



                        @if(!empty($payments[0]))
                            <a class="btn btn-xs btn-success"  href="{{ route('invoice_history_pdfview',['download'=>'pdf','id'=>$invoices->id]) }}"  title="" > Download Payment History </a>
                        @endif

                        @can('view-delivery-menu')
                            @if(!empty($payments[0]))
                                <a class="btn btn-xs btn-primary" href="{{ route('show_delivery', ['id' => $invoices->id]) }}" title="Delivery">
                                    Delivery
                                </a>
                            @endif
                        @endcan
                                ...
                                <div class="tab-pane fade show active" id="about" role="tabpanel"
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-3 col-xs-3">
                                            ...
                                            <br>Delivery Date : {{Carbon\Carbon::parse($invoices->delivery_date)->format('d/m/Y')}}
                            ...
                            <div class="table-responsive mb-lg">
                                <table class="table items invoice-items-preview" page-break-inside:="" auto;="">
                                    ...
                                    <tr>
                                        ...
                                        <th style="color:white;">Original Qty</th>
                                        <th style="color:white;">Balance Qty</th>
                                    ...
                                    <tr>
                                        <td colspan="5"></td>
                                        <td>Sub Total</td>
                                        <td>{{number_format($sub_total > 0 ? $sub_total : $invoices->invoice_amount,2)}}  {{$invoices->exchange_code}}</td>
                                    </tr>

                                    <tr>
                                        <td colspan="5"></td>
                                        <td>Total Tax </td>
                                        <td>{{number_format($tax > 0 ? $tax : $invoices->invoice_tax,2)}}  {{$invoices->exchange_code}}</td>
                                    </tr>
                                    ...
                                    @if(isset($invoices->discount) && $invoices->discount > 0)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{number_format((($sub_total > 0 ? $sub_total : $invoices->invoice_amount) + ($tax > 0 ? $tax : $invoices->invoice_tax) + ($invoices->shipping_cost ?? 0)) - $invoices->discount,2)}}  {{$invoices->exchange_code}}</td>
                                        </tr>
                                    @endif
                                    ...
                                    @if($invoices->adjustment !=0)
                                        <tr>
                                            ...
                                            <td>{{number_format((($sub_total > 0 ? $sub_total : $invoices->invoice_amount) + ($tax > 0 ? $tax : $invoices->invoice_tax) + ($invoices->shipping_cost ?? 0)) - ($invoices->discount ?? 0),2)}}  {{$invoices->exchange_code}}</td>
                                        </tr>
                                        <tr>
                                            <td colspan="5"></td>
                                            <td>Adjustment</td>
                                    ...
                                    <tr>
                                        ...
                                        <td><strong>{{number_format((($sub_total > 0 ? $sub_total : $invoices->invoice_amount) + ($tax > 0 ? $tax : $invoices->invoice_tax) + ($invoices->shipping_cost ?? 0)) - ($invoices->discount ?? 0) + ($invoices->adjustment ?? 0),2)}}  {{$invoices->exchange_code}}</strong></td>
                                    </tr>
                                    ...
                                    @if($invoices->status != 1 && $invoices->status != 4 &&  $invoices->invoice_status == 1)
                                        <tr>
                                            <td colspan="5"></td>
                                            <td>Paid Amount</td>
                                            <td>{{number_format((($sub_total > 0 ? $sub_total : $invoices->invoice_amount) + ($tax > 0 ? $tax : $invoices->invoice_tax) + ($invoices->shipping_cost ?? 0)) - ($invoices->discount ?? 0) + ($invoices->adjustment ?? 0) - $invoices->due_amount,2)}}  {{$invoices->exchange_code}}</td>
                                        </tr>
                                    ...
                                    @if($invoices->exchange_code != $def->currency)
                                        ...
                                        @if($invoices->status != 1 && $invoices->status != 4 &&  $invoices->invoice_status == 1)
                                            <tr>
                                                <td colspan="5"></td>
                                                <td>Paid Amount</td>
                                                <td>{{number_format( $invoices->exchange_rate * (((($sub_total > 0 ? $sub_total : $invoices->invoice_amount) + ($tax > 0 ? $tax : $invoices->invoice_tax) + ($invoices->shipping_cost ?? 0)) - ($invoices->discount ?? 0) + ($invoices->adjustment ?? 0)) - $invoices->due_amount),2)}} {{$def->currency}}</td>
                                            </tr>
                                    ...
                                    @if(!@empty($invoices->notes))
                                        <tr>
                                            <td colspan="7">Terms and Conditions : <br>{{$invoices->notes}}</td>
                                        </tr>
                                    @endif
                                    </tfoot>
                                </table>
                                        ...
                                        <table class="table datatable-basic table-striped">
                                            ...
                                            @foreach($payments as $row)
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="">{{ $row->payment->account_name ?? 'Default Account' }}</td>
