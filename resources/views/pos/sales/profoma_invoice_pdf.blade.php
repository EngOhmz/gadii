...
<style type="text/css">
    ...
    footer {
        color: #000;
        width: 100%;
        height: 40px; /* Increased height to accommodate page number */
        position: absolute;
        bottom: -15px;
...
<body>
    ...
    $bank_details = App\Models\SystemDetails::find($invoices->bank_details_id);
    ?>
    <div class="watermark p-0"> {{ is_null($invoices->status) ? 'DRAFT' : ($invoices->status == 0 ? 'PROFORMA' : $invoices->status) }} </div>
                ...
                <tr>
                    ...
                    <td class="w-50">
                        <div class="box-text">
                            ...
                            <p style="font-size: 12px;"><strong>P/Invoice Validity::</strong> {{ Carbon\Carbon::parse($invoices->delivery_date)->format('d/m/Y') }}</p>
                        </div>
                    </td>
                </tr>
            </tfoot>
                ...
                <tr>
                    ...
                    <td>
                        <div class="box-text">
                            ...
                            @php
                                $clientTin = $invoices->client->TIN ?? $invoices->client->tin ?? '';
                                $clientVrn = $invoices->client->VRN ?? $invoices->client->vrn ?? '';
                            @endphp
    ...
    <div class="table-section bill-tbl w-100 mt-10">
        @if (!empty($invoices->profoma_attachment))
            <p style="font-size: 12px; margin-bottom: 6px;">
                ...
                <a href="{{ asset($invoices->profoma_attachment) }}" target="_blank">Download Attachment</a>
            </p>
        @endif
        ...
        <table class="table w-100 mt-10">
            <thead>
                <tr>
                    ...
                    <th style="background-color: #00008B; color: white;">Total + VAT [{{ $invoices->exchange_code }}]</th>
                </tr>
            </thead>
            <tbody>
                @if (!empty($invoice_items))
                    @foreach ($invoice_items as $row)
                        ...
                        <tr align="center">
                            ...
                            <td style="text-align: right;">{{ $item_name->unit }}</td>
        ...
        <div class="">
            <img src="{{ url('public/assets/img/signature') }}/{{ $settings->signature }}" alt="{{ $settings->signature }}" width="80">
