...
$supp=App\Models\Supplier::find($row->purchase->supplier_id);

?>
                                            <td class=""> {{$row->trans_id}}</td>
                                               <td class="">{{Carbon\Carbon::parse($row->date)->format('d/m/Y')}}  </td>
                                                <td>{{$supp->name}}</td>
                                            <td class="">{{ number_format($row->amount ,2)}} {{$row->purchase->exchange_code}}</td>
                                            <td class="">{{ $method->name }}</td>
                                          <td class="">{{ $row->payment->account_name }}</td>
                                            <td>  <a class="nav-link"  href="{{ route('payment_pdfview',['download'=>'pdf','id'=>$row->id]) }}"  title="" > Download PDF </a> </td>
                                        </tr>
                                        @endforeach
