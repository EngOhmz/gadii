                                        ...
                                        <thead>
                                            <tr role="row">
                                             ...
                                             <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0"
                                                    ...
                                                    style="width: 96.484px;">Reference</th>
                                                <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0"
                                                    rowspan="1" colspan="1"
                                                    aria-label="Platform(s): activate to sort column ascending"
                                                ...
                                                <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0"
                                                    rowspan="1" colspan="1"
                                                    aria-label="CSS grade: activate to sort column ascending"
                                                    style="width: 158.1094px;">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @if(!@empty($issue))
                                            ...
                                            <div class="form-inline">
                                               ...
                                               <a class="list-icons-item text-success"
                                                    href="{{ route("pos_disposal.approve", $row->id)}}" onclick="return confirm('Are you sure, You want to Approve')" title="Change Status">
                                                    <i class="icon-checkmark3"></i>
                                                </a>&nbsp&nbsp                                             

                                                    <a class="list-icons-item text-primary"
                                                        href="{{ route("disposal.edit", $row->id)}}">
                                                        <i class="icon-pencil7"></i>
                                                    </a>&nbsp
                                                   
                                                    {!! Form::open(['route' => ['disposal.destroy',$row->id],
                                                    'method' => 'delete']) !!}
                                                  {{ Form::button('<i class="icon-trash"></i>', ['type' => 'submit', 'style' => 'border:none;background: none;', 'class' => 'list-icons-item text-danger', 'title' => 'Delete', 'onclick' => "return confirm('Are you sure?')"]) }}
                                                    {{ Form::close() }}

                                                    @else
                                                    <a class="nav-link"href=""  data-toggle="modal" href=""  value="{{ $row->id}}" data-type="issue" data-target="#appFormModal"
                                                        onclick="model({{ $row->id }},'disposal')">
                                                        View  Items
                                                    </a>
                                               @endif
</div>
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-sm-12 ">
                                                     @if(isset($id))
                                                {{ Form::model($id, array('route' => array('disposal.update', $id), 'method' => 'PUT')) }}
                                                @else
                                                {{ Form::open(['route' => 'disposal.store']) }}
                                                @method('POST')
                                                @endif
                                 <div class="form-group row">
                                                ...
                                                <label class="col-lg-2 col-form-label">Location</label>
                                                    <div class="col-lg-4">
                                                        <select class="form-control m-b location" name="location" id="location_id" required>
                                                ...
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-lg-4">
                                                        <textarea name="description" class="form-control" required>{{isset($data)? $data->description : ''}}</textarea>
                                                    </div>
    ...
    $(document).ready(function() {
        ...
        $('.add').on("click", function(e) {
            ...
            html += '<tr class="line_items">';                  
             html += '<td><select name="item_id[]" class="form-control m-b item_name" required  data-sub_category_id="' + count + '"><option value="">Select Item</option>@foreach($inventory as $n) <option value="{{ $n->id}}">{{$n->name}}</option>@endforeach</select></td>';
        html +='<td><input type="number" name="quantity[]" class="form-control item_quantity" data-category_id="' +count + '"placeholder ="quantity" id ="quantity" value= "" required /> <div class=""> <p class="form-control-static errors'+count+'" id="errors" style="text-align:center;color:red;"></p>   </div></td>';
...
<script>
    ...
    console.log(location);
            $.ajax({
                url: '{{url("pos/purchases/findDQuantity")}}',
                type: "GET",
                data: {
                    id: id,
                  item: item,
...
$.ajax({
    type: 'GET',
     url: '{{url("pos/purchases/disposalModal")}}',
    data: {
        'id': id,
        'type':type,
    },
