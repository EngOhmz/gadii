        ...
        <tbody>
                           ...
                           $due=App\Models\POS\PurchaseHistory::where('purchase_id',$row->purchase_id)->where('item_id',$row->item_name)->where('type', 'Purchases')->sum('quantity');
                            $return=App\Models\POS\PurchaseHistory::where('purchase_id',$row->purchase_id)->where('item_id',$row->item_name)->where('type', 'Debit Note')->sum('quantity');
                             $qty=$due-$return;
                              ...
                              $gland_total +=($row->price * $qty) + (($row->price * $qty) * $row->tax_rate);
                              $tax += ($row->price * $qty) * $row->tax_rate; 
                                         ?>
            ...
            <tr align="center">
                ...
                <td >{{ number_format($qty,2) }}</td> 
                 ...
                 <td>  {{number_format( ($row->price * $qty) * $row->tax_rate ,2)}} </td>                           
                <td >{{number_format( $row->price * $qty ,2)}} </td>
...
<tr>
         ...
         <div><b>ISSUED BY </div> 
         <div>@if(!empty($purchases->assign->name)){{$purchases->assign->name }} @endif</div>
          </div>  </td>
