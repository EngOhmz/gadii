...
@php
  $due=App\Models\POS\PurchaseHistory::where('purchase_id',$purchases->id)->where('type', 'Purchases')->where('added_by',auth()->user()->added_by)->first();
@endphp
              ...
              <a class="btn btn-xs btn-info"  title="Approve" onclick="return confirm('Are you sure?')"    href="{{ route('purchase.receive',$purchases->id)}}"  title="" >Approve Purchase </a>
                @endif
     ...
     <a class="btn btn-xs btn-info"  data-type="receive" onclick="model({{ $purchases->id  }},'receive')" href=""  data-toggle="modal" data-target="#appFormModal"> Good Receive </a>   

 @if(!empty($due))        
              <a class="btn btn-xs btn-info"    href="{{ route('purchase.issue',$purchases->id)}}"  onclick="return confirm('Are you sure?')">Issue Supplier </a>
                @endif
...
@if( $purchases->good_receive == 1)
   <a class="btn btn-xs btn-success"  href="{{ route('purchase_issue_pdfview',['download'=>'pdf','id'=>$purchases->id]) }}"  title="" > Download Supplier Issue </a>      
   @endif  
...
?>
                ...
                <img class="pl-lg" style="width: 240px;height: 120px;" src="{{url('assets/img/logo')}}/{{$settings->picture}}">
            </div>
 ...
 <div class="col-lg-3 col-xs-3">
                                      ...
                                      <div class="col-lg-3 col-xs-3">
                                        <h5 class="mb0">HEADING : {{ $purchases->purchase_heading ?? 'No Heading.' }}</h5>
                                       <h5 class="mb0">REF NO : {{$purchases->reference_no}}</h5>
                                   @if(!empty($purchases->project_id)) Project No : {{$purchases->project->project_name}} - {{$purchases->project->project_no}} @endif
                                     ...
                                     <br>Due Date : {{Carbon\Carbon::parse($purchases->due_date)->format('d/m/Y')}}                                          
                                       <br>Purchase Agent: @if(!empty($purchases->assign->name)){{$purchases->assign->name }} @endif
                        ...
                        <th style="color:white;">Total</th>
                                    ...
                                    <tbody>
                                        ...
                                        <?php
                                         ...
                                         $due=App\Models\POS\PurchaseHistory::where('purchase_id',$purchases->id)->where('item_id',$row->item_name)->where('type', 'Purchases')->sum('quantity');
                                      $return=App\Models\POS\PurchaseHistory::where('purchase_id',$purchases->id)->where('item_id',$row->item_name)->where('type', 'Debit Note')->sum('quantity');
                                                          $qty=$due-$return;
                                         ?>
                                        <tr>
                                            <td class="">{{$i++}}</td>
...
?>
                                            ...
                                            <td> <a href="{{ route('payment_pdfview',['download'=>'pdf','id'=>$row->id]) }}"  title="" > Download Receipt </a> </td> 
                                        </tr>
                                        @endforeach
