...
<div class="head-title">
    <h1 class="text-center m-0 p-0">Payment History</h1>
</div>
            ...
            <tr align="center">
                <td>{{$i++}}</td>
                 <?php
                                        ...
                                        ?>
                <td> @if(!empty($item_name->name)) {{$item_name->name}} @if(!empty($item_name->color)) - {{$item_name->c->name}} @endif   @if(!empty($item_name->size)) - {{$item_name->s->name}} @endif @else {{$row->item_name}}  @endif <br>{{$row->description}}   </td>
