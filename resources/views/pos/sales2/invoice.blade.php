                                                ...
                                                @if (!@empty($invoices))
                                                    @foreach ($invoices as $row)
                                                        ...
                                                        <tr class="gradeA even" role="row">
                                                            ...
                                                            <td>
                                                            <a href="{{ route('invoice2.show', $row->id) }}">{{ $row->reference_no }}</a>
                                                            </td>
                                                   ...
                                                   ?>
                                                    ...
                                                    <div class="form-inline">
                                                        ...
                                                        @can('approve-edit')
                                                        @if (empty($rn))
                                                        <a class="list-icons-item text-primary" title="Edit" onclick="return confirm('Are you sure?')" href="{{ route('invoice2.edit', $row->id) }}">
                                                        ...
                                                        @can('delete-sales')
                                                        @if (empty($rn))
                                                        {!! Form::open(['route' => ['invoice2.destroy', $row->id], 'method' => 'delete']) !!}
                                                        ...
                                                        @if ($row->status != 0 && $row->status != 4 && $row->status != 3 && $row->good_receive == 1)
                                                          <li> <a class="nav-link" id="profile-tab2" href="{{ route('pos_invoice.pay', $row->id) }}">Make Payments</a></li>
                                                        @endif
                                                                            
                    <a class="nav-link" data-toggle="modal"  onclick="attach_model({{$row->id}},'attach')" data-target="#attachFormModal" data-id="{{$row->id}}" href="attachFormModal">Attachments</a>
                     <a class="nav-link" data-toggle="modal"  onclick="model2({{$row->id}},'commission')" data-target="#app2FormModal" data-id="{{$row->id}}" href="app2FormModal">Commission</a>
                                                                               

                            <a class="nav-link" id="profile-tab2" href="{{ route('pos_invoice_pdfview', ['download' => 'pdf', 'id' => $row->id]) }}">Download PDF</a>
                             <a class="nav-link" id="profile-tab2" href="{{ route('pos_invoice_receipt', ['download' => 'pdf', 'id' => $row->id]) }}">Download Receipt</a>
                             
                              @if($row->status == 3 || $row->status == 2)
                 <a class="nav-link"  href="{{ route('invoice_history_pdfview',['download'=>'pdf','id'=>$row->id]) }}"  title="" > Download Payment History </a>
                   @endif 
                   
                    <a class="nav-link" id="profile-tab2" target="_blank" href="{{ route('pos_invoice_print', ['download' => 'pdf', 'id' => $row->id]) }}">Print PDF</a>
                    <a class="nav-link" id="profile-tab2" target="_blank" href="{{ route('pos_receipt_print', ['download' => 'pdf', 'id' => $row->id]) }}">Print Receipt</a>
                                                                        </div>
                                                                    </div>
                                    ...
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <div class="row">
                                                <div class="col-sm-12 ">
                                                    @if (isset($id))
                                                {{ Form::model($id, array('route' => array('invoice2.update', $id), 'method' => 'PUT',"enctype"=>"multipart/form-data", 'id' => 'invform')) }}
                                                        
                                                    @else
                                                        {!! Form::open(array('route' => 'invoice2.store',"enctype"=>"multipart/form-data", 'id' => 'invform')) !!}
                                                        @method('POST')
                                                    @endif
                                                    ...
                                                    <div class="form-group row">
                                                        ...
                                                        <div class="col-lg-4">
                                                            ...
                                                            <select class="form-control m-b sales" name="sales_type"
                                                                ...
                                                                <option value="Cash Sales"
                                                                    @if (isset($data)) {{ $data->sales_type == 'Cash Sales' ? 'selected' : '' }} @else selected @endif>
                                                                    Cash Sales</option>
                                                        ...
                                                        @else
                                                            <label for="stall_no" class="col-lg-2 col-form-label bank1">Bank/Cash Account <span class="required"> * </span></label>
                                                    ...
                                                    <textarea name="notes" class="form-control" rows="4">{{ isset($data) ? $data->notes : '' }}</textarea>
                                                         ...
                                                         <div class="form-group row">
                                                        <label class="col-lg-2 col-form-label">Attachment <br><small> You can upload a maximum of 10 files</small></label>
                                                        <div class="col-lg-8">
...
<h5>Add attachment</h5> <small> You can upload a maximum of 10 files</small>
     ...
     <form id="addForm" method="post" action="{{route('save_attachment')}}" enctype="multipart/form-data">
                                                                @csrf

                                                    <div class="form-group row">
                                                        <div class="col-lg-6">
                                                    <div class="needsclick dropzone modal-dz" id="modal-dz" style="min-width:10px;min-height:10px;"></div>
                                                        </div>
                                                         ...
                                                         <br><br><br>   
                        <button class="btn btn-primary"  type="submit"><i class="icon-checkmark3 font-size-base mr-1"></i>Upload</button>
                                                         </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('.add').on("click", function(e) {
            ...
            var html = '';
            html +=
                 '<div class="col-lg-3 line_items" id="td'+count + '"><br><div class="input-group mb-3"><select name="checked_item_name[]" class="form-control m-b item_name"  id="item_name' +
                count + '" data-sub_category_id="' + count +'" required><option value="">Select Item Name</option>@foreach ($name as $n) <option value="{{ $n->id }}">{{ $n->name }} @if(!empty($n->color)) - {{$n->c->name}} @endif   @if(!empty($n->size)) - {{$n->s->name}} @endif</option>@endforeach</select></div><div class="c261b1ca9 c26517808"><span>or</span></div><div><a href="#" class="col-lg-12 btn btn-outline-secondary text-center scan"  data-toggle="modal" data-target="#appFormModal" data-sub_category_id="' +count +'"><i class="icon-barcode2"> </i> Scan</a></div><br><div id="upd'+count + '" style="display:none; "><a href="#" class="col-lg-12 btn btn-outline-info text-center update"  data-toggle="modal" data-target="#appFormModal" data-sub_category_id="' +count +'">Update Quantity</a></div><br><textarea name="checked_description[]"  class="form-control desc' + count +'" placeholder="Description"  ></textarea><br></div>';
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        if( b == '' || c == '' || d == '' || e == ''){
         ...
         $.ajax({
                type: 'GET',
                 url: '{{ url('pos/sales/add_inv_item2') }}',
