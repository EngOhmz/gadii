    ...
    <div class="section-body">
      ...
      @if($invoices->status == 0 && $invoices->status != 4 && $invoices->status != 3 && $invoices->good_receive == 0)                        
           ...
           @endif  
            ...
            <a class="btn btn-xs btn-success"  href="{{ route('orders_receipt',['download'=>'pdf','id'=>$invoices->id]) }}">Download Order Receipt</a>  
                                            @else
            <a class="btn btn-xs btn-success"  href="{{ route('orders_receipt',['download'=>'pdf','id'=>$invoices->id]) }}">Download Bill Receipt</a> 
                                            @endif
...
<tr>
    ...
    <td>{{number_format($gland_total  - $invoices->discount ,2)}}  {{$invoices->exchange_code}}</td>
     ...
     @if(!empty($invoices->due_amount < ($invoices->invoice_amount + $invoices->invoice_tax )  - $invoices->discount))
         <tr>
    <td colspan="5"></td>
                        <td>Paid Amount</p>
                        <td>{{number_format(( ($invoices->invoice_amount + $invoices->invoice_tax )  - $invoices->discount) - $invoices->due_amount,2)}}  {{$invoices->exchange_code}}</p>
                    </tr>
<td colspan="5"></td>
...
?>
                                      ...
                                      <td class="">{{ $row->payment->account_name }}</td>
                                         <td> <a  href="{{ route('orders_payment_pdfview',['download'=>'pdf','id'=>$row->id]) }}"  title="" > Download Receipt </a></td>   
                                        </tr>
                                        @endforeach
