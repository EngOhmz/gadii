     <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title"  style="text-align:center;"> {{$key->name}} @if(!empty($key->color)) - {{$key->c->name}} @endif   @if(!empty($key->size)) - {{$key->s->name}} @endif Issued Quantity<h5>
  ...
  <div class="table-responsive">
                           <table class="table datatable-basic table-striped">
                              ...
                              @elseif($a->type == 'Good Disposal') @php $dr = App\Models\POS\GoodDisposal::find($a->other_id); @endphp @if(!empty($dr)) {{$dr->name }}@endif
                                             ...
                                             <td >
                                             @if($a->type == 'Good Issue')  {{ number_format($a->out ,2) }} 
                                            @elseif($a->type == 'Returned Good Issue') {{ number_format(0-$a->in ,2) }}
                                              @endif
