    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title"  style="text-align:center;"> {{$key->name}}  Disposed Quantity<h5>
 ...
 <tbody>
                    <?php                   
                          $account =App\Models\GoodDisposalItem::leftJoin('good_disposals', 'good_disposals.id','good_disposal_items.disposal_id')
                                                          ->where('brand_id', $key->id)
                                                           ->whereIn('good_disposals.location',$loc_id)
                                                           ->where('good_disposal_items.status',1)     
                                                             ->whereBetween('date',[$start_date,$end_date])    
                                                           ->select('good_disposals.*','good_disposal_items.*')
                                                              ->get()  ;
                        ?>  
 ...
 <?php
                  ...
                  $q =App\Models\GoodDisposalItem::leftJoin('good_disposals', 'good_disposals.id','good_disposal_items.disposal_id')
                                                          ->where('brand_id', $key->id)
                                                           ->whereIn('good_disposals.location',$loc_id)
                                                           ->where('good_disposal_items.status',1)     
                                                             ->whereBetween('date',[$start_date,$end_date])    
                                                           ->select('good_disposals.*','good_disposal_items.*')
                                                              ->sum('good_disposal_items.quantity')  ;
