        ...
        <div class="row">
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="gradeA even" role="row">
                                                ...
                                                <td>
                                                    <a class="nav-link" id="profile-tab2"
                                                            href="{{ route('inventory_credit_note.show',$row->id)}}" role="tab"
                                            ...
                                            <div class="form-inline">
                                                    ...
                                                    <a class="list-icons-item text-primary"
                                                        ...
                                                        href="{{ route('inventory_credit_note.edit', $row->id)}}"><i
                                                            class="icon-pencil7"></i></a>&nbsp
                                                            @endif

                                                    {!! Form::open(['route' => ['inventory_credit_note.destroy',$row->id],
                                                    'method' => 'delete']) !!}
          ...
          @if($row->status == 0)
                                                            <li> <a class="nav-link" id="profile-tab2"
                                                                    href="{{ route('inventory_credit_note.approve',$row->id)}}"
                                                                    role="tab"
                                                            ...
                                                            <li> <a class="nav-link" id="profile-tab2"
                                                                    href="{{ route('inventory_credit_note.receive',$row->id)}}"
                                                                    role="tab"
                                                            ...
                                                            <li> <a class="nav-link" id="profile-tab2"
                                                                    href="{{ route('inventory_credit_note.pay',$row->id)}}"
                                                                    role="tab"
                                                            ...
                                                            <li class="nav-item"><a class="nav-link" title="Cancel"
                                                                    ...
                                                                    href="{{ route('inventory_credit_note.cancel', $row->id)}}">Cancel
                                                                  Credit Note</a></li>
                                        @endif
                                       </div>
                                       </div>
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-sm-12 ">
                                                @if(isset($id))
                                                {{ Form::model($id, array('route' => array('inventory_credit_note.update', $id), 'method' => 'PUT')) }}
                                              
                                                @else
                                                {{ Form::open(['route' => 'inventory_credit_note.store']) }}
                                                @method('POST')
                                                @endif
              ...
              <div class=""><p class="form-control-static errors" id="errors" style="text-align:center;color:red;"></p>   </div>
                                            ...
                                            <div class="table-responsive" id="crd-cart">
                                    ...
                                    @foreach($name as $n) 
                                    <option value="{{ $n->id}}"
                            @if(isset($i))@if($n->id == $i->item_name) selected @endif @endif >{{$n->brand->name}} - {{ $n->serial_no }}</option>
                                        @endforeach
                                                                    </select>
                                    <br> </div>
                    ...
                    <input type="hidden" name="quantity[]" class="form-control item_quantity"  min="1" placeholder="quantity" id="quantity" data-sub_category_id="{{$i->id}}_qty" value="{{ isset($i) ? $i->quantity : ''}}" required="">
...
console.log(id);
        $.ajax({
            url: '{{url("inventory/inventory_findinvoice")}}',
            type: "GET",
            data: {
                id: id
            },
...
console.log(id);
        $.ajax({
            url: '{{url("inventory/inventory_editshowInvoice")}}',
            type: "GET",
            data: {
                id: id,
            },
            ...
            success: function(response) {
                ...
                $.each(response,function(key, value)
                {
                     ...
                     $('#crd-cart > .body').append(response.html);
...
$(document).ready(function() {
    ...
    function autoCalcSetup() {
        $('div#crd-cart').jAutoCalc('destroy');
        $('div#crd-cart div.line_items').jAutoCalc({
            keyEventsFire: true,
            decimalPlaces: 2,
            emptyAsZero: true
        });
        $('div#crd-cart').jAutoCalc({
            decimalPlaces: 2
        });
    }
    autoCalcSetup();
...
console.log(id);
        $.ajax({
            url: '{{url("inventory/inventory_findinvQty")}}',
            type: "GET",
            data: {
                id: id,
              item: item,
    ...
    $(document).ready(function() {
         ...
         $('.errors').empty();
          ...
          if ( $('#crd-cart > .body .line_items').length == 0 ) {
               event.preventDefault(); 
    $('.errors').append('Please Add Items.');
}
