...
<?php $total_qty=0; ?>
             @if(!empty($purchase_items))
                           ...
                           $due=App\Models\Tyre\TyreHistory::where('purchase_id',$row->purchase_id)->where('item_id',$row->item_name)->where('type', 'Purchases')->sum('quantity');
                            $return=App\Models\Tyre\TyreHistory::where('purchase_id',$row->purchase_id)->where('item_id',$row->item_name)->where('type', 'Debit Note')->sum('quantity');
                             $qty=$due-$return;
                              $total_qty += $qty 
                                         ?>
            ...
            <tr align="center">
                <td>{{$i++}}</td>
                 <?php
                                        ...
                                        ?>
                <td>@if(!empty($item_name->brand)) {{$item_name->brand}} @else {{$row->item_name}} @endif   </td>             
