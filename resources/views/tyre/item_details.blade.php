    <div class="modal-content">
        <div class="modal-header">
            ...
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                ...
                <form id="form" role="form" enctype="multipart/form-data" action="{{route('tyre.grn')}}"  method="post" >
                                                ...
                                                <table class="table table-bordered" id="cart">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        @foreach ($purchase_items as $i)
                                                           ...
                                                           <?php
                                                             $due=App\Models\Tyre\TyreHistory::where('purchase_id',$id)->where('item_id',$i->item_name)->where('type', 'Purchases')->sum('quantity');
                                                         $return=App\Models\Tyre\TyreHistory::where('purchase_id',$id)->where('item_id',$i->item_name)->where('type', 'Debit Note')->sum('quantity');
                                                          $qty=$due-$return;
                                                              ?>
                                                    @if( $i->quantity - $qty > 0)
                                                        <tr class="line_items">
