            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4>Manage Student Class</h4>
                    </div>
                    <div class="card-body">
                        <ul class="nav nav-tabs">
                            <li class="nav-item">
                                <a class="nav-link @if(!isset($id)) active @endif" data-toggle="tab" href="#list">Student Classes List</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link @if(isset($id)) active @endif" data-toggle="tab" href="#tab2">Add/Edit Student Class</a>
                            </li>
                        </ul>
                        ...
                        <div class="tab-content pt-3">
                            ...
                            <div class="tab-pane fade @if(!isset($id)) active show @endif" id="list">
                                <div class="table-responsive">
                                    <table class="table datatable-basic table-striped">
                                        ...
                                        <tbody>
                                            @if(!empty($studentsclasses))
                                            @foreach($studentsclasses as $studentsclass)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $studentsclass->name }}</td>
                                                <td>{{ $studentsclass->level->name ?? 'N/A' }}</td>
                                                <td>{{ $studentsclass->status }}</td>
                                                <td>
                                                    <div class="form-inline">
                                                        <a class="list-icons-item text-primary" href="{{ route('studentsclass.edit', $studentsclass->id) }}">
                                                            <i class="icon-pencil7"></i>
                                                        </a>
                                                        &nbsp;
                                                        {!! Form::open(['route' => ['studentsclass.destroy', $studentsclass->id], 'method' => 'delete']) !!}
                            ...
                            <div class="tab-pane fade @if(isset($id)) active show @endif" id="tab2">
                                <div class="card-body">
                                    @if(isset($id))
                                        {!! Form::model($data, ['route' => ['studentsclass.update', $id], 'method' => 'PUT']) !!}
                                    @else
                                        {!! Form::open(['route' => 'studentsclass.store', 'method' => 'POST']) !!}
                                    @endif
                                    ...
                                    <div class="form-group text-right">
                                        ...
                                        <a href="{{ route('studentsclass.index') }}" class="btn btn-secondary">Cancel</a>
                                    </div>
