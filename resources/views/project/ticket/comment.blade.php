                ...
                @if (!empty($comment_details))
                    @foreach ($comment_details as $v_comment)
                        ...
                        <div class="hstack align-items-start gap-3">
                            ...
                            <div class="flex-fill">
                                <div>
                                    ...
                                    <span class="text-muted fs-sm ms-2"> &nbsp;&nbsp;
                                        {{ \Carbon\Carbon::parse($v_comment->comment_datetime)->diffForHumans() }}</span>
                                </div>
                                {{ $v_comment->comment }}
                            </div>

                            <br>
                            <button style="border:none;background: none;" class="text-primary reply"
                                data-toggle="tooltip" data-placement="top" title="Click to reply"
                                id="reply__<?php echo $v_comment->id; ?>" data-sub_category_id="<?php echo $v_comment->id; ?>"><i
                                    class="icon-reply"></i> Reply</button> &nbsp
                            @if ($v_comment->user_id == auth()->user()->id)
                                <a class="text-danger delete" title="Edit"
                                    href="{{ route('delete.project_details', ['type' => 'delete-comments', 'type_id' => $v_comment->id]) }}"
                            ...
                            <?php
                            $comment_reply = App\Models\Project\Comment::where('project_id', $id)
                                ->where('comments_reply_id', $v_comment->id)
                                ->where('disabled', '0')
                                ->orderBy('comment_datetime', 'ASC')
                                ->get();
                            ?>
                                            ...
                                            <div class="col-lg-10"><br>
                                                <div class="hstack align-items-start gap-3">
                                                    ...
                                                    <div class="flex-fill">
                                                        <div>
                                                            <a href="#"
                                                                class="fw-semibold">{{ $v_reply->user->name }}</a>
                                                            <span class="text-muted fs-sm ms-2"> &nbsp;&nbsp;
                                                                {{ \Carbon\Carbon::parse($v_reply->comment_datetime)->diffForHumans() }}</span>
                                                        </div>
                                                        {{ $v_reply->comment }}
                                                    </div>
                                                    ...
                                                    @if ($v_reply->user_id == auth()->user()->id)
                                                        <a class="text-danger delete" title="Edit"
                                                            href="{{ route('delete.project_details', ['type' => 'delete-comments', 'type_id' => $v_reply->id]) }}"
                                                            onclick="return confirm('Are you sure, you want to delete?')"><i
                                                                class="icon-trash"></i> Delete</a>
                                                    @endif
                            ...
                            <div class="reply__" id="reply_<?php echo $v_comment->id; ?>" style="display: none;">
                                <form id="projects-reply-comment-form-<?php echo $v_comment->id; ?>"
                                    action="{{ route('save.project_details') }}" method="post">
                                    @csrf
                                    <input type="hidden" name="project_id" value="{{ $id }}"
                                        class="form-control">
                                    ...
                                    <button type="button" style="border:none;background: none;"
                                        class="text-danger remove" data-toggle="tooltip" data-placement="top"
                                        title="" id="cancel-<?php echo $v_comment->id; ?>" data-original-title="Remove"
                                        data-category_id="<?php echo $v_comment->id; ?>"><i class="icon-x"></i> Remove </button>
