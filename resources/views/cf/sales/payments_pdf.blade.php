    ...
    <div>
        <h4 align="center" style="margin-top: 0%;" class="fontS">{{$settings->name}}<h4>
         ...
         @if(!empty($settings->TIN)) <br>TIN : {{ $settings->TIN}} @endif
                                    ...
                                    @if($purchases->related == 'Clients')
                                     @php $name= $purchases->client->name; @endphp
                                    @else 
                                     @php $name = App\Models\Departments::find($purchases->client_id)->name; @endphp
                                    @endif 
  

                <hr style="border: 1px solid;">  
                <p align="center" style="font-weight: normal;" class="fontL">
                <b> Invoice Ref No :{{ $purchases->reference_no }}</b>
                ...
                <br>Amount: {{ number_format($data->amount)}} {{$purchases->exchange_code}}
