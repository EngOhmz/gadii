...
<div class="panel panel-white col-lg-12">
            ...
            <table id="data-table" class="table table-striped ">
                    ...
                    <tr>
                 <th>#</th>
                        <th colspan="3" style="text-align: center">STATEMENT OF FINANCIAL POSITION FOR THE PERIOD ENDING {{ $start_date }} </th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr>
                       ...
                       $total_credit_liability  =    $total_credit_liability + ($cr1-$dr1) +$net_profit['tax_for_second_date'];
                        ...
                        ?>                           
                         ...
                         @else
                            ...
                            <td>{{ number_format( ($cr1+$net_profit['tax_for_second_date']) - $dr1,2) }}</td>
                        </tr>
                         @endif
...
<thead>
 ...
 <tbody>
                    <?php    
                         ...
                         if(!empty($branch_id) && $branch_id != $a){
                         $account = \App\Models\JournalEntry::where('account_id', $account_code->id)->whereIn('branch_id', $br_id)->where('date', '<=',
                            $start_date)->where('added_by',auth()->user()->added_by)->orderBy('date','desc')->get();
                            
                         }else{
                            
                             $account = \App\Models\JournalEntry::where('account_id', $account_code->id)->where('date', '<=',
                            $start_date)->where('added_by',auth()->user()->added_by)->orderBy('date','desc')->get();
...
<thead>
 ...
 <?php
                         $vat_in = \App\Models\AccountCodes::where('account_name', 'VAT IN')->where('added_by',auth()->user()->added_by)->first();
                            ...
                            if(!empty($branch_id) && $branch_id != $a){
                         $account = \App\Models\JournalEntry::where('account_id', $vat_in->id)->whereIn('branch_id', $br_id)->where('date', '<=',
                            $start_date)->where('added_by',auth()->user()->added_by)->orderBy('date','desc')->get();
                            
                         }else{
                            
                               $account = \App\Models\JournalEntry::where('account_id', $vat_in->id)->where('date', '<=',
                            $start_date)->where('added_by',auth()->user()->added_by)->orderBy('date','desc')->get();
...
<thead>
 ...
 <?php
                         $vat_out = \App\Models\AccountCodes::where('account_name', 'VAT OUT')->where('added_by',auth()->user()->added_by)->first();
                              ...
                              if(!empty($branch_id) && $branch_id != $a){
                         $account_out = \App\Models\JournalEntry::where('account_id', $vat_out->id)->whereIn('branch_id', $br_id)->where('date', '<=',
                            $start_date)->where('added_by',auth()->user()->added_by)->orderBy('date','desc')->get();
                            
                         }else{
                            
                              $account_out = \App\Models\JournalEntry::where('account_id', $vat_out->id)->where('date', '<=',
                            $start_date)->where('added_by',auth()->user()->added_by)->orderBy('date','desc')->get();
...
<div class="modal fade" id="view{{$account_code->id}}"  tabindex="-1" role="dialog" aria-hidden="true">
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
  <div class="table-responsive">
                            <table class="table datatable-li table-striped">
